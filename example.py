			...
			if transform_matrix is not None:
				...
				if len(self.trajectory) == 0:
					...
					self.trajectory.append(
						[prev_traj[0] + dx, prev_traj[1] + dy, prev_traj[2] + da]
					)
				...
				if len(self.smoothed_trajectory) > 0:
					...
					stabilized_frame = self._crop_borders(
						stabilized_frame, crop_ratio=0.1
					)
...
def stabilize_frame(
	frame: np.ndarray, stabilizer: FrameStabilizer = None
) -> Tuple[np.ndarray, FrameStabilizer]:
	"""
