...
def show_message(self, type: MessageType, message: str):
    ...
    msg = MessageBox(title=type, content=message, parent=self)
...
class ConfigDialog(QDialog):
        ...
        for key, value in data.items():
            ...
            if isinstance(value, dict):
                ...
                if prefix:  # Don't add spacing before first section
                    spacer = QSpacerItem(
                        0, 16, QSizePolicy.Policy.Minimum, QSizePolicy.Policy.Fixed
                    )
    ...
    def save_config(self):
        try:
            ...
            show_message(
                self, MessageType.ERROR, f"Failed to save configuration:\n{str(e)}"
            )
...
class LoginWindow(QMainWindow):
    def __init__(self, accept=None):
        ...
        if accept is not None:
            ...
            self.accept = lambda: show_message(
                self, MessageType.INFO, "Login successful!"
            )
        self.dialog = ConfigDialog(self)

        self.setup_ui()

    def set_on_login_success(self, callback):
    ...
    def create_login_form(self, main_layout):
        ...
        subtitle = QLabel(
            self.dialog.config_data.get("app", {}).get(
                "description", "A control center for Matek drones"
            )
        )
    ...
    def handle_login(self):
        ...
        if not username or not password:
            show_message(
                self, MessageType.WARNING, "Please enter both username and password."
            )
            return

        if username == self.dialog.config_data.get("app", {}).get(
            "user", ""
        ) and password == self.dialog.config_data.get("app", {}).get("password", ""):
            # Simulate login
            self.accept()
            self.close()
