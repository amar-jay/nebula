...
def train_model(data, weights, epochs, batch_size, img_size):
	...
	)  # TODO: find a less expensive way to do check GPU
...
def run_inference(model_path, camera_path=0):
	...
	cap = cv2.VideoCapture(
		camera_path
	)  # Change the argument for other cameras if needed
...
if __name__ == "__main__":
	parser = argparse.ArgumentParser(
		description="Fine-tune & detect YOLOv8 for Helipad & Tank Detection."
	)
	...
	parser.add_argument(
		"--video", type=int, default=0, help="Path to an video cam for inference."
	)
	parser.add_argument("--weights", type=str, help="Path to a YOLO model weights")
