...
class AntennaTracker:
	def __init__(self, latitude, longitude):
		...
		self.antenna_lat, self.antenna_lon, self.antenna_alt = (
			latitude,
			longitude,
			0.0,
		)  # AntennaTracker'ın sabit koordinatları
	...
	def send_servo_angles(self):
		...
		self.arduino.write(
			changed_x.to_bytes(1, "little") + changed_y.to_bytes(1, "little") + b"\n"
		)
	...
	def input_vehicle_gps(self):
		vehicle_lat, vehicle_lon = input(
			"vehicle'ın enlem ve boylamını girin: "
		).split()
...
def update_heading(pixhawk):
	...
	if msg:
		...
		print(
			"VFR_HUD mesajı alınamadı. Eğer default konumu almadıysa program tekrar başlatılmalı!"
		)
		return None
...
def antenna_tracker(antenna, vehicle):
	...
	try:
		...
		pixhawk = mavutil.mavlink_connection(
			"/dev/ttyACM0", baud=115200, autoreconnect=True
		)
		try:
		...
		while connected:
			try:
				if time.time() - last_time > delay_interval:
					heading = update_heading(pixhawk)
					antenna.track(
						heading, vehicle.latitude, vehicle.longitude, vehicle.altitude
					)
					last_time = time.time()
					time.sleep(0.05)
