...
class CameraWidget(QWidget):
	def __init__(self, parent=None):
		...
		self.labels_checkbox = QCheckBox(
			parent=self, styleSheet="background-color: transparent;color: blue;"
		)
	...
	def connectStream(self):
		ip, okPressed = QInputDialog.getText(
			self, "Enter IP Address", "IP Address:", text="192.168.1.1"
		)
		if okPressed:
	...
	def resizeEvent(self, event):
		...
		self.connect_button.move(
			int((self.width() - self.connect_button.width()) / 2),
			int((self.height() - self.connect_button.height()) / 2),
		)
