...
def updateTargetPosition(mainwindow, no, position):
	mainwindow.homepage.mapwidget.page().runJavaScript(
		f"target_marker{no}.setLatLng({str(position)});"
	)
...
class VideoStreamThread(QThread):
	...
	def __init__(
		self, parent=None, ip=socket.gethostbyname(socket.gethostname()), port=5050
	):
		super().__init__()
		self.parent = parent
		self.ip = ip
		...
		self.p2 = (
			int(self.parent.width() - self.parent.width() // 6),
			int(self.parent.height() // 2),
		)
			...
			try:
				...
				for det in detections:
					if det["track_id"] not in self.saved_detections:
						...
						self.NewTargetDetectedSignal.emit(
							target_image,
							det["position"],
							[time.time(), time.time()],
							det["track_id"],
					...
					self.UpdateTargetPositionSignal.emit(
						self.parent.parent.parent, det["track_id"], det["position"]
					)
	...
	def setHorizon(self, roll):
		...
		self.p2 = (
			int(self.parent.width() - x + length * math.cos(roll)),
			int(y - length * math.sin(roll)),
		)
	...
	def setImageBorders(self, detection):
		if detection["bb_width"] > detection["bb_height"]:
			...
			detection["bb_height"] = (
				detection["bb_height"] + detection["bb_height"] / 10
			)
