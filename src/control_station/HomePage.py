...
class HomePage(QWidget, Ui_HomePage):
	def __init__(self, parent=None):
		...
		self.mapwidget.btn_AllocateWidget.clicked.connect(
			lambda: self.AllocateWidget(self.mapFrame, self.mapwidget)
		)
		self.cameraWidget.btn_AllocateWidget.clicked.connect(
			lambda: self.AllocateWidget(self.cameraFrame, self.cameraWidget)
		)
	...
	def buttonFunctions(self):
		...
		if button.objectName() == "btn_chooseMode":
			if self.modes_comboBox.currentText() == "İşaretçi Modu":
				self.mapwidget.page().runJavaScript(
					f"map.on('click', moveMarkerByClick);"
				)
				self.mapwidget.page().runJavaScript(f"map.off('click', drawRectangle);")
				self.mapwidget.page().runJavaScript(
					f"map.off('click', putWaypointEvent);"
				)
			if self.modes_comboBox.currentText() == "Alan Seçimi Modu":
				...
				self.mapwidget.page().runJavaScript(
					f"map.off('click', moveMarkerByClick);"
				)
			...
			if self.modes_comboBox.currentText() == "Waypoint Modu":
				...
				self.mapwidget.page().runJavaScript(f"map.off('click', drawRectangle);")
				self.mapwidget.page().runJavaScript(
					f"map.on('click', putWaypointEvent);"
				)
	...
	def set_mission(self):
		altitude, okPressed = QInputDialog.getText(
			self, "Enter Altitude", "Altitude:", text="10"
		)
		...
		if okPressed:
			if self.modes_comboBox.currentText() == "Waypoint Modu":
				...
				QTimer().singleShot(
					1000,
					lambda: self.parent.connectionThread.set_mission(
						MissionModes.WAYPOINTS, self.mapwidget.mission, altitude
					),
				)
			else:
				...
				QTimer().singleShot(
					1000,
					lambda: self.parent.connectionThread.set_mission(
						MissionModes.EXPLORATION, self.mapwidget.mission, altitude
					),
				)
