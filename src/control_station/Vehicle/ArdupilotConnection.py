...
def updateData(thread, vehicle, mapwidget, indicators, camerawidget, firebase):
	...
	if msg is not None:
		...
		if msg.get_type() == "GLOBAL_POSITION_INT":
			...
			# Update UAV marker
			mapwidget.page().runJavaScript(
				f"uavMarker.setLatLng({str(position)});"
			)  # to set position of UAV marker
			mapwidget.page().runJavaScript(
				f"uavMarker.setRotationAngle({heading - 45});"
			)  # to set rotation of UAV
		...
		if msg.get_type() == "SYS_STATUS":
			...
			thread.parent.label_top_info_1.setText(
				f"Battery: {msg.battery_remaining}%      {msg.voltage_battery / 1e3}V      {msg.current_battery}A"
			)
...
class ArdupilotConnectionThread(QThread):
	...
	updateData_signal = Signal(
		QThread, mavutil.mavudp, MapWidget, IndicatorsPage, CameraWidget, FirebaseUser
	)
	...
	def rtl(self):
		def control_if_reached():
			if (
				abs(self.latitude - self.home_position[0]) > 0.0001
				or abs(self.longitude - self.home_position[1]) > 0.0001
			):
