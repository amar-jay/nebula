...
def exploration(vehicle, point1, point2, altitude, fov):
	...
	# Find first waypoint
	starting_point_no = find_closest_point(
		[vehicle.latitude, vehicle.longitude], point_list
	)
	ending_point_no = (starting_point_no + 2) % 4

	# Find Short Edge
	short_edge = find_short_edge(
		point1, point2
	)  # 0 for horizontal, 1 for vertical edge
	if short_edge == vertical:
		short_edge_length = get_distance_from_lat_lon_in_km(
			point_list[up_left], point_list[down_left]
		)
	else:
		short_edge_length = get_distance_from_lat_lon_in_km(
			point_list[up_left], point_list[up_right]
		)

	# Calculate Distance Between Waypoint (in short edge)
	distance_between_wp = calculate_distance_between_waypoints(
		fov, vehicle.camera_angle, altitude
	)
	...
	if short_edge == horizontal:
		...
		if starting_point_no == up_right or starting_point_no == down_right:
			direction = 180
		_, new_lng = get_point_at_distance(
			point_list[starting_point_no][0],
			point_list[starting_point_no][1],
			90 + direction,
		...
		new_lat, _ = get_point_at_distance(
			point_list[starting_point_no][0],
			point_list[starting_point_no][1],
			180 + direction,
			distance_between_wp / 2,
	...
	while (i // 2) * distance_between_wp < (
		short_edge_length - distance_between_wp / 2
	):
		if short_edge == vertical:
...
def find_short_edge(point1, point2):
	...
	) < get_distance_from_lat_lon_in_km([0, point1[1]], [0, point2[1]]):
		return True
	else:
		return False
...
def get_distance_from_lat_lon_in_km(point1, point2):
	...
	a = math.sin(dLat / 2) * math.sin(dLat / 2) + math.cos(
		deg2rad(point1[0])
	) * math.cos(deg2rad(point2[0])) * math.sin(dLon / 2) * math.sin(dLon / 2)
