...
class Firebase:
	def __init__(self):
		cred = credentials.Certificate(
			r"uifolder/assets/fir-demo-31f2b-firebase-adminsdk-75i4b-a17ad191f3.json"
		)
	...
	def add_user_to_database(self, name, image, activity, coordinates):
		...
		c.execute(
			"""INSERT INTO users (name, image, activity, x, y)
                     VALUES (?, ?, ?, ?, ?)""",
			(name, image, activity, coordinates["x"], coordinates["y"]),
		)
