...
import controls.mavlink as mavlink
...
class DroneClient(QObject):
    ...
    def connect_to_drone(self, address, port, is_kamikaze=False):
        ...
        if is_kamikaze:
            ...
            self.master_connection = mavutil.mavlink_connection(
            f"udp:{address}:{port}"
            )
        ...
        self.connection_status.emit(True, f"[MAVLink] Connected to {address}:{port} for {'Kamikaze' if is_kamikaze else 'Drone'}")
    ...
    def disconnect(self, is_kamikaze=False):
        ...
        if self.connected:
            ...
            self.mission_active = False
            self.status_timer.stop()
            self.connection_status.emit(False, f"[MAVLink] Disconnecting from drone at {self.tcp_address}:{self.tcp_port}")
    ...
    def arm(self, is_kamikaze=False):
        ...
        mavlink.gz.arm(self.master_connection if not is_kamikaze else self.kamikaze_connection)
        ...
        location = mavlink.gz.get_current_gps_location(self.master_connection if not is_kamikaze else self.kamikaze_connection)
        if location is None:
    ...
    def takeoff(self, altitude):
        ...
        mavlink.gz.takeoff(self.master_connection, altitude)
        self.flying = True
    ...
    def return_to_home(self):
        ...
        mavlink.gz.return_to_launch(self.master_connection)
    ...
    def goto_coordinates(self, lat, lon, alt):
        ...
        mavlink.gz.goto_waypoint(self.master_connection, curr_lat+lat, curr_lon+lon, alt)
    ...
    def start_mission(self):
        ...
        for i, waypoint in enumerate(self.mission_waypoints):
            ...
            self.mission_progress.emit(
                int((i + 1) * 100 / len(self.mission_waypoints)), 
                msg
            )
    ...
    def _update_status(self):
        ...
        if self.flying and self.battery > 10:
            self.battery -= 1
        status = mavlink.gz.get_status(self.master_connection)    
        # status = {
...
class MissionWaypointTable(QTableWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.setHorizontalHeaderLabels(["#", "Latitude", "Longitude", "Altitude (m)", "Actions"])
    ...
    def add_waypoint(self, index, lat, lon, alt):
        ...
        delete_btn.clicked.connect(lambda: self.removeRow(self.indexAt(delete_btn.pos()).row()))
...
class DroneControlApp(QMainWindow):
    ...
    def _on_connect_clicked(self):
        ...
        if self.drone_client.connect_to_drone(address, port):
            ...
            self.console.append_message(f"Failed to connect to {address}:{port}", "error")
    ...
    def _on_goto_clicked(self):
        ...
        if self.drone_client.goto_coordinates(lat, lon, alt):
            self.console.append_message(f"Moving to position: Lat {lat}, Lon {lon}, Alt {alt}m", "success")
        else:
    ...
    def _on_add_waypoint_clicked(self):
        ...
        self.console.append_message(f"Added waypoint {index}: Lat {lat}, Lon {lon}, Alt {alt}m", "info")
    ...
    def _on_upload_mission_clicked(self):
        ...
        if self.drone_client.upload_mission(waypoints):
            self.console.append_message(f"Uploaded mission with {len(waypoints)} waypoints", "success")
