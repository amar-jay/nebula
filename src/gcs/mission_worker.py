...
class MissionWorker(QObject):
    ...
    operation_completed = Signal(
        str, bool
    )  # Emitted when operation completes (operation, success)
    ...
    def start_sequential_operation(self):
        ...
        self.timeout_requested.emit(
            130000
        )  # 130 seconds - emit signal for thread safety
    ...
    def _complete_operation(self, operation: ZMQTopics, success: bool):
        ...
        with self._operation_lock:
            ...
            self._log(
                f"Completed {operation.value} in {execution_time:.2f}s (success: {success})"
            )
    ...
    def _handle_timeout(self):
        ...
        with self._operation_lock:
            ...
            self._log(
                f"Operation {operation.value} timed out after 10 seconds", "warning"
            )
...
class MissionWorkerManager(QObject):
    ...
    def _on_operation_completed(self, operation: str, success: bool):
        ...
        if self.logger:
            self.logger(
                f"Completed {operation} {status}", "success" if success else "warning"
            )
...
class KamikazeWorker(QObject):
    ...
    def start_kamikaze_sequence(
        self, target_coordinates: Tuple[float, float], takeoff_altitude: float = 10.0
    ):
        """
    ...
    def _execute_kamikaze_sequence(
        self, target_coordinates: Tuple[float, float], takeoff_altitude: float
    ):
        """
        ...
        try:
            steps = [
                ...
                (
                    "Executing kamikaze strike",
                    lambda: self._execute_kamikaze(target_coordinates),
                ),
