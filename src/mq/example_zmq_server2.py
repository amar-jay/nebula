...
class MAVLinkBridge:
    ...
    def __init__(self, is_simulation: bool):
        ...
        self.running = False
        self.executor = ThreadPoolExecutor(max_workers=MAX_CLIENTS, thread_name_prefix="client")
    ...
    def get_drone_state(self) -> Tuple[Optional[Dict], Optional[Dict]]:
        ...
        try:
            ...
            gps_msg = self.connection.recv_match(type='GLOBAL_POSITION_INT', blocking=False)
            if gps_msg:
                gps = {
                    'lat': gps_msg.lat / 1e7,
                    'lon': gps_msg.lon / 1e7,
    ...
    def start(self):
        ...
        self.forward_thread = threading.Thread(target=self.forward_to_clients, daemon=True)
...
class ZMQServer:
    ...
    def __init__(
        ...
        logger.info(f"ZMQ Server initialized - Video: {video_port}, Control: {control_port}")
    ...
    def _start_capture(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Video capture started: {self.frame_width}x{self.frame_height}")
            return True
        except Exception as e:
            logger.error(f"Error starting video capture: {e}")
            return False
    
    def _encode_frame(self, frame: np.ndarray, frame_type: str = "raw") -> Tuple[bytes, bytes]:
    ...
    def _process_frame(self, frame: np.ndarray) -> Tuple[np.ndarray, Optional[Dict]]:
        ...
        try:
            ...
            # Process with YOLO
            annotated_frame, detected_coords, center_pose = self.estimator.process_frame(
                frame=frame,
                drone_gps=gps,
    ...
    def _video_loop(self):
        ...
        while self.running:
            try:
                ...
                topic, processed_encoded = self._encode_frame(processed_frame, "processed")
...
def main():
    ...
    parser.add_argument("--weights-path", default="yolo_weights.pt", help="YOLO weights")
