...
class ZMQVideoThread(QThread):
    ...
    def setup_zmq(self):
        ...
        try:
            ...
            self.video_socket.setsockopt_string(zmq.SUBSCRIBE, "")  # Subscribe to all topics
            logger.info(f"Connected to video stream at {self.server_ip}:{self.video_port}")
...
class ZMQClient:
    ...
    def __init__(self, server_ip="localhost", video_port=5555, control_port=5556):
        ...
        self.video_thread.processed_frame_received.connect(self._on_processed_frame_received)
    ...
    def send_command(self, command) -> str:
        ...
        try:
            ...
            self.control_socket.send_string(command.name if hasattr(command, 'name') else str(command))
