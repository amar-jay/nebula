...
class ModernGauge(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.max_val != self.min_val:
            value_angle = (self.value - self.min_val) / (self.max_val - self.min_val) * span_angle
            painter.setPen(QPen(self.get_color_for_value(self.value), 8, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
            painter.drawArc(center.x() - radius + 15, center.y() - radius + 15, 
                           (radius - 15) * 2, (radius - 15) * 2, 
                           start_angle * 16, -value_angle * 16)
        ...
        painter.drawText(center.x() - text_rect.width()//2, center.y() + 5, value_text)
        
        # Unit text
        painter.setFont(QFont("Arial", 9))
        painter.setPen(QPen(self.text_color.darker(150)))
        unit_rect = painter.fontMetrics().boundingRect(self.unit)
        painter.drawText(center.x() - unit_rect.width()//2, center.y() + 20, self.unit)
